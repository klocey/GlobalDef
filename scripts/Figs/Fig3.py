...
df = pd.read_csv(mydir+'/DataSets/Distances/DataSets.txt')
df = df[df['distance'] > 0]
...
for name in namesDF:
    ...
    print(name, ' : ', round(np.mean(ds),3), ' : ', round(np.std(ds),3),
          round(np.max(ds),3))
...
plt.axhline(10007.614102966581, 0, 100, color=hline_clr1, linewidth=2, ls='--')
plt.axhline(8110.541893322524, 0, 100, color=hline_clr2, linewidth=2, ls='--')

for i, m in enumerate(range(len(names))):
    #print('Data:',names[i], 'mean:',mean[i])
...
plt.axhline(19010579.645256173, 0, 100, color=hline_clr1, linewidth=2, ls='--')
plt.axhline(15385763.879415138, 0, 100, color=hline_clr2, linewidth=2, ls='--')
...
plt.axhline(0.0, 0, 100, color=hline_clr1, linewidth=2, ls='--')
plt.axhline(0.25434830941313213, 0, 100, color=hline_clr2, linewidth=2, ls='--')
